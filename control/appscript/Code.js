...
/** The five status values, in the order they appear in the dropdown. */
var STATUS_VALUES = ['Not Started', 'In Progress', 'Complete', 'Deficiency', 'On Hold'];
 ...
 * A phase or a unit shows the first status found in this order.
 * If none are found, everything is Complete.
 */
...
/** Returns one unit's items, grouped by phase, with status and details. */
function handleGetUnit(id, unitKey) {
  ...
  var phases = activePhases(config).map(function (phase) {
    var items = phase.items.map(function (item) {
      var status = statusKey(values[layout.statusCol[item.key] - 1]);
      itemStatuses.push(status);
      return {
        key:     item.key,
        label:   item.label,
        status:  status,
        details: String(values[layout.detailsCol[item.key] - 1] || '')
      };
    });
...
/**
 * Changes a project's structure. Never touches a status or a details value.
 *
 * Every operation follows the same three steps:
 *   1. Read every status and details value that already exists.
 *   2. Change the config.
...
function applyStructureOp(config, data) {
  ...
  if (data.op === 'add-item') {
    ...
    phase.items.push({ key: uniqueKey(slug(label), taken), label: label });
    return null;
  }
...
function buildConfig(data) {
  ...
  (data.phases || []).forEach(function (phase) {
    var items = (phase.items || []).map(function (item) {
      ...
      return { key: uniqueKey(slug(label), takenItemKeys), label: label };
 ...
 * Column 1  = Floor
 * Column 2  = Unit #
 * Then one Status column and one Details column for each item, in order.
 * Then one rollup column for each phase.
 * Then Last Updated, then Overall Status.
 *
 * With 18 items and 4 phases this gives exactly the master template's
 * layout: status columns C to AK, rollups AM to AP, AQ Last Updated,
 * AR Overall Status.
 */
function computeLayout(config) {
  ...
  phases.forEach(function (phase) {
    phase.items.forEach(function (item) {
      ...
      layout.detailsCol[item.key] = col + 1;
 ...
 *
 * "preserved" is optional. It maps 'unitKey|itemKey' to { status, details },
 * and 'unitKey|__updated' to the Last Updated value. Anything missing starts
 * at Not Started with empty details.
 */
function rebuildTracker(ss, config, preserved) {
  ...
  layout.order.forEach(function (itemKey) {
    ...
    sheet.setColumnWidth(layout.detailsCol[itemKey], 145);
  });
  phases.forEach(function (phase) {
  ...
  phases.forEach(function (phase) {
    ...
    var to        = layout.detailsCol[lastItem.key];

    sheet.getRange(4, from, 1, to - from + 1).merge()
         .setValue(bandLabel(phase.label));

    phase.items.forEach(function (item) {
      sheet.getRange(5, layout.statusCol[item.key], 1, 2).merge()
  ...
  // -- Row 6, sub-headers ----------------------------------------------
  var head = ['Floor', 'Unit #'];
  layout.order.forEach(function () { head.push('Status', 'Details'); });
  ...
  var rows = units.map(function (unit, index) {
    ...
    layout.order.forEach(function (itemKey) {
      ...
      cells[layout.detailsCol[itemKey] - 1] = saved ? saved.details : '';
    });

    phases.forEach(function (phase) {
      var cols = phase.items.map(function (item) { return layout.statusCol[item.key]; });
      cells[layout.rollupCol[phase.key] - 1] = buildRollupFormula(cols, row);
    });
  ...
  // rollup block. Five rules in total.
...
function rebuildDashboard(ss, config) {
  ...
  resizeSheet(sheet, rowCount, 6);
  sheet.setHiddenGridlines(true);

  sheet.setColumnWidth(1, 225);
  for (var c = 2; c <= 6; c++) sheet.setColumnWidth(c, 159);
  sheet.setRowHeight(1, 40);
  sheet.setRowHeight(2, 24);
  ...
  sheet.getRange(rowSummary, 1, 1, 6).merge()
       .setValue('  PROGRESS SUMMARY  (units, out of ' + units.length + ')');

  sheet.getRange(rowOverall, 1, 1, 6).merge()
 ...
 *
 * Worst status wins:
 *   any Deficiency   -> Deficiency
 *   else any On Hold -> On Hold
 *   else any In Progress -> In Progress
 *   else any Not Started -> Not Started
 *   else Complete
 *
 * Complete shows only when every item is Complete.
 *
 * ONE DELIBERATE CHANGE FROM THE MASTER TEMPLATE.
 * The template's last test asks whether the Not Started count equals the
 * item count. That makes a half-done unit read "Complete": three items
 * Complete and three Not Started gives 3, not 6, so the formula falls
 * through to Complete. That is wrong, and it is the kind of wrong answer
 * that makes a tracker untrustworthy.
 *
 * This version asks whether the Not Started count is above zero, so one
 * unstarted item is enough to hold the whole phase at Not Started. It now
 * matches the written rule and the app's own worst() helper exactly.
 */
function buildRollupFormula(cols, row) {
  ...
  return '=IF((' + countOf('Deficiency')  + ')>0,"Deficiency",' +
          'IF((' + countOf('On Hold')     + ')>0,"On Hold",' +
          'IF((' + countOf('In Progress') + ')>0,"In Progress",' +
          'IF((' + countOf('Not Started') + ')>0,"Not Started","Complete"))))';
}


// ── SHEET READ HELPERS ────────────────────────────────────────────────

/** Reads every status and details value, so a rebuild can put them back. */
  ...
  units.forEach(function (unit, index) {
    ...
    layout.order.forEach(function (itemKey) {
      saved[unit.key + '|' + itemKey] = {
        ...
        details: String(row[layout.detailsCol[itemKey] - 1] || '')
      };
    });
...
/** Worst status wins. Takes short keys, returns a short key. */
function worst(keys) {
  if (!keys || keys.length === 0) return 'not_started';
  for (var i = 0; i < ROLLUP_ORDER.length; i++) {
    if (keys.indexOf(ROLLUP_ORDER[i]) !== -1) return ROLLUP_ORDER[i];
  }
  return 'complete';
}
...
// connection. Writing the same value twice must be safe, and it is: the
// call carries the final value, not a change to apply.
//
// Everything it needs already exists here:
